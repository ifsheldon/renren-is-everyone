import { getSubtitleFiles, getFileContent, isDirectory } from '@/lib/files';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    return {
        title: `${decodedPath} - Subtitle Server`,
        description: `View ${decodedPath}`,
    };
}
    ...
    if (isDir) {
        ...
        return (
            <div className="min-h-screen p-8 sm:p-20 font-[family-name:var(--font-geist-sans)] bg-gradient-to-br from-gray-900 to-black text-white relative">
                <LanguageSwitcher lang="en" currentPath={`/file/${relativePath}`} />
                <main className="max-w-7xl mx-auto">
                    ...
                    <FileExplorer files={files} currentPath={relativePath} />
                </main>
            </div>
        );
    }
            ...
            <main className="w-full flex-1 flex flex-col min-h-0">
                <header className="mb-6 flex items-center justify-between">
                    <div className="flex items-center gap-4 min-w-0">
                        <Link
                            href={parentPath ? `/file/${parentPath}` : '/'}
