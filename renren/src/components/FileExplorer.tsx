...
export function FileExplorer({ files, currentPath = '', lang = 'en' }: Props) {
    ...
    const t = {
        ...
        noFiles: lang === 'zh' ? '此目录下没有文件。' : 'No files found in this directory.',
    };
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {files.length === 0 ? (
                    ...
                    files.map((file) => (
                        ...
                        >
                            <div className={`p-3 rounded-lg ${file.isDirectory ? 'bg-yellow-500/20 text-yellow-400' : 'bg-blue-500/20 text-blue-400'} group-hover:scale-110 transition-transform`}>
                                {file.isDirectory ? (
